...
class FourDEnVar:
        ...
        for filename in self.p_dict.keys():
            for nml in self.p_dict[filename].keys():
                for param in self.p_dict[filename][nml].keys():
                    if type(self.p_dict[filename][nml][param][0]) == list:
                        for val in self.p_dict[filename][nml][param]:
                            self.p_keys.append(param)
                            self.xb.append(val[1])
                            self.val_bnds.append(val[2])
                    else:
                        self.p_keys.append(param)
    ...
    def make_obs(self):
        ...
        obs_dic = es.obs_fn()
        self.yoblist = obs_dic['obs']
        self.yerr = obs_dic['obs_err']
    ...
    def cost(self, xi, xi_nc):
        """
        Calculates cost function for given xi
        :param xi: parameter vector (arr)
        :param xi_nc: netcdf jules output file (str)
        :return: value of cost fn. (float)
        """
        modcost = np.dot((xi-self.xb), np.dot(np.linalg.inv(self.b_mat),(xi-self.xb).T))
        hxi = es.jules_hxi(xi_nc)['obs']
        obcost = np.dot(np.dot((hxi - self.yoblist), np.linalg.inv(self.rmatrix)),
                      (hxi - self.yoblist).T)
