...
class RunJulesDa:
    ...
    def __init__(self, params=['neff_io'], values='default', nml_dir='example_nml', year=2008):
        ...
        self.p_dict = cp.copy(es.opt_params)
                    ...
                    for param in self.p_dict[filename][nml].keys():
                        ...
                        if type(self.p_dict[filename][nml][param][0]) == list:
                            if param in self.p_keys:
                                idxs = np.where(np.array(self.p_keys)==param)[0]
                                print idxs
                                for i in xrange(len(idxs)):
                                    self.p_dict[filename][nml][param][i][1] = values[i]
                        else:
                            if param in self.p_keys:
                ...
                for param in self.p_dict[filename][nml].keys():
                    if type(self.p_dict[filename][nml][param][0]) == list:
                        for val in self.p_dict[filename][nml][param]:
                            j.nml_dic[filename][nml][param][val[0]] = val[1]
                    elif type(self.p_dict[filename][nml][param][0]) == int:
